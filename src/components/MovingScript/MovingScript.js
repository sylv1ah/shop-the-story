import React from 'react';
import './MovingScript.css';

const MovingScript = () => (
    <div className="movingscript">
        <div className="moving script1">
            <i className="names">Stacy</i> 
            <i> yesterday’s dinner was so…hey excuse me, why are you taking my flower? </i>
            <i className="names">Ji-Yeon</i>
            <i> six times three divided by eight is… </i>
            <i className="names">Muk Ya</i>
            <i> this will look better with my purple bed sheets anyway. </i>
            <i className="names">Felix</i>
            <i> not my problem that you are not intellectual enough to understand my narrative…  Cemine My followers will love this week’s flower arrangement</i>
        </div>
        {/* <p className="moving script2">
        <i></i>
        </p> */}
        
    </div>
);

export default MovingScript;